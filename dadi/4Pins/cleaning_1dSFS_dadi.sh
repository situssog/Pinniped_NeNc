...
grep -A 4 "Optimized parameters" 1dsfs_data_$out_file.bootstrap | tr -s " " | grep -v "^\-\-$" | grep -v ", array" | grep -v "^[0-9]*$" | tr -d '\n' | sed -e 's/$/\n/' -e 's/Optimized/\nOptimized/g' | sed -e 's/$/\n/' -e 's/Bootstrap/\nBootstrap/g' | grep -v "uncertainties" | sed 's/Optimized //g' | tr -d '\n' | sed -e 's/$/\n/' -e 's/parameters/\nparameters/g' | sed '/^$/d' | sed 's/\[//g;s/\]//g' | sed 's/[)(]//g' | sed 's/parameters array //g' | sed 's/parameters array//g'  | tr -s " " | sed 's/log-likelihood: /, /g' | sed 's/ ,/,/g' | sed 's/,//g' | grep -v inf | grep -v "\-\-" > 1dsfs_data_$out_file.bootstrap.clean_withlikelihood
